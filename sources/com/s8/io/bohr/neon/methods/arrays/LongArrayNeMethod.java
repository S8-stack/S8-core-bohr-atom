package com.s8.io.bohr.neon.methods.arrays;

import java.io.IOException;

import com.s8.io.bohr.BOHR_Types;
import com.s8.io.bohr.neon.core.NeBranch;
import com.s8.io.bohr.neon.core.NeObjectPrototype;
import com.s8.io.bohr.neon.methods.NeFunc;
import com.s8.io.bohr.neon.methods.NeMethod;
import com.s8.io.bohr.neon.methods.NeRunnable;
import com.s8.io.bytes.alpha.ByteInflow;

/**
 * 
 *
 * @author Pierre Convert
 * Copyright (C) 2022, Pierre Convert. All rights reserved.
 * 
 */
public class LongArrayNeMethod extends NeMethod {


	public interface Lambda {
		
		public void operate(long[] arg);
		
	}
	

	public final static long SIGNATURE = (BOHR_Types.ARRAY << 8) & BOHR_Types.INT64;

	public @Override long getSignature() { return SIGNATURE; }

	
	/**
	 * 
	 * @param prototype
	 * @param name
	 */
	public LongArrayNeMethod(NeObjectPrototype prototype, String name) {
		super(prototype, name);
	}



	@Override
	public NeRunnable buildRunnable(ByteInflow inflow) throws IOException {
		if(inflow.getUInt8() != BOHR_Types.ARRAY) {	throw new IOException("Must be an array"); }
		switch(inflow.getUInt8()) {
		
		case BOHR_Types.UINT8 : new UInt8ArrayNeRunnable();
		case BOHR_Types.UINT16 : new UInt16ArrayNeRunnable();
		case BOHR_Types.UINT32 : new UInt32ArrayNeRunnable();
		case BOHR_Types.UINT64 : new UInt64ArrayNeRunnable();
		
		case BOHR_Types.INT8 : new Int8ArrayNeRunnable();
		case BOHR_Types.INT16 : new Int16ArrayNeRunnable();
		case BOHR_Types.INT32 : new Int32ArrayNeRunnable();
		case BOHR_Types.INT64 : new Int64ArrayNeRunnable();
		
		default : throw new IOException("Unsupported type");
		}
	}
	

	private static class UInt8ArrayNeRunnable implements NeRunnable {

		@Override
		public void run(NeBranch branch, ByteInflow inflow, NeFunc func) throws IOException {
			int length = (int) inflow.getUInt7x();
			if(length >= 0) {
				long[] arg = new long[length];
				for(int i=0; i<length; i++) { arg[i] = inflow.getUInt8(); }
				((Lambda) (func.lambda)).operate(arg);
			}
			else {
				((Lambda) (func.lambda)).operate(null);
			}
		}
	}
	


	private static class UInt16ArrayNeRunnable implements NeRunnable {

		@Override
		public void run(NeBranch branch, ByteInflow inflow, NeFunc func) throws IOException {
			int length = (int) inflow.getUInt7x();
			if(length >= 0) {
				long[] arg = new long[length];
				for(int i=0; i<length; i++) { arg[i] = inflow.getUInt16(); }
				((Lambda) (func.lambda)).operate(arg);
			}
			else {
				((Lambda) (func.lambda)).operate(null);
			}
		}
	}
	


	private static class UInt32ArrayNeRunnable implements NeRunnable {

		@Override
		public void run(NeBranch branch, ByteInflow inflow, NeFunc func) throws IOException {
			int length = (int) inflow.getUInt7x();
			if(length >= 0) {
				long[] arg = new long[length];
				for(int i=0; i<length; i++) { arg[i] = inflow.getUInt32(); }
				((Lambda) (func.lambda)).operate(arg);
			}
			else {
				((Lambda) (func.lambda)).operate(null);
			}
		}
	}
	

	private static class UInt64ArrayNeRunnable implements NeRunnable {

		@Override
		public void run(NeBranch branch, ByteInflow inflow, NeFunc func) throws IOException {
			int length = (int) inflow.getUInt7x();
			if(length >= 0) {
				long[] arg = new long[length];
				for(int i=0; i<length; i++) { arg[i] = inflow.getUInt64(); }
				((Lambda) (func.lambda)).operate(arg);
			}
			else {
				((Lambda) (func.lambda)).operate(null);
			}
		}
	}
	


	private static class Int8ArrayNeRunnable implements NeRunnable {

		@Override
		public void run(NeBranch branch, ByteInflow inflow, NeFunc func) throws IOException {
			int length = (int) inflow.getUInt7x();
			if(length >= 0) {
				long[] arg = new long[length];
				for(int i=0; i<length; i++) { arg[i] = inflow.getInt8(); }
				((Lambda) (func.lambda)).operate(arg);
			}
			else {
				((Lambda) (func.lambda)).operate(null);
			}
		}
	}
	


	private static class Int16ArrayNeRunnable implements NeRunnable {

		@Override
		public void run(NeBranch branch, ByteInflow inflow, NeFunc func) throws IOException {
			int length = (int) inflow.getUInt7x();
			if(length >= 0) {
				long[] arg = new long[length];
				for(int i=0; i<length; i++) { arg[i] = inflow.getInt16(); }
				((Lambda) (func.lambda)).operate(arg);
			}
			else {
				((Lambda) (func.lambda)).operate(null);
			}
		}
	}
	



	private static class Int32ArrayNeRunnable implements NeRunnable {

		@Override
		public void run(NeBranch branch, ByteInflow inflow, NeFunc func) throws IOException {
			int length = (int) inflow.getUInt7x();
			if(length >= 0) {
				long[] arg = new long[length];
				for(int i=0; i<length; i++) { arg[i] = inflow.getInt32(); }
				((Lambda) (func.lambda)).operate(arg);
			}
			else {
				((Lambda) (func.lambda)).operate(null);
			}
		}
	}
	
	


	private static class Int64ArrayNeRunnable implements NeRunnable {

		@Override
		public void run(NeBranch branch, ByteInflow inflow, NeFunc func) throws IOException {
			int length = (int) inflow.getUInt7x();
			if(length >= 0) {
				long[] arg = new long[length];
				for(int i=0; i<length; i++) { arg[i] = inflow.getInt64(); }
				((Lambda) (func.lambda)).operate(arg);
			}
			else {
				((Lambda) (func.lambda)).operate(null);
			}
		}
	}
	
}
